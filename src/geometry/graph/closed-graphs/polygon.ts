 ...
 * @LastEditor: theajack
 * @LastEditTime: 2021-09-11 16:36:51
...
export class Polygon extends Graph implements IPolygon {
    ...
    countIntersectPointNumberWithLine (anotherLine: LineSegment) {
        let sum = 0;
        this.edges.forEach((line: LineSegment, index: number) => {
    ...
    private prevLine (index: number|LineSegment): LineSegment {
    ...
    countIntersectPointNumberWithLines (edges: LineSegment[]) {
        const sums: number[] = [];
    ...
    private isAnotherLineCrossEdge (anotherLine: LineSegment, edge: LineSegment, edgeIndex?: number) {
